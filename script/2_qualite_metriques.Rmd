...
Ce volet a pour but d'étudier la distribution globale à l'échelle de la région d'un certain nombre d'indicateurs hydromorphologiques susceptibles de mettre en évidence des disfonctionnements des cours d'eau de la région. Ces distributions devront permettre, via la création de classes de qualité hydromorphologique, de statuer sur l'état hydromorphologique des stations Carhyce de la région au regard des métriques étudiées. 
...
Qb_Q2_der_ope$version_protocole <- ordered(
  Qb_Q2_der_ope$version_protocole, 
  levels = c("2009 (Pré-déploiement)", "2009-2016", "2017"))
...
Cette métrique peut être déclinée de deux manière : en étudiant les valeurs brutes du rapport largeur/profondeur de plein bord, ou en étudiant les résidus aux modèles de référence du rapport largeur/profondeur de plein bord (autrement dit l'écart aux modèles de référence). Cette deuxième manière d'analyser le rapport L/P est possible car le rapport L/P de plein bord est une métrique de l'IMG (Indice Morphologique Global). Il dispose donc d'un modèle linéaire de référence construit à partir des stations de référence pour l'hydromorphologie en fonction de la surface de leur bassin versant. 
...
Cela indique a priori que les cours d'eau au niveau de ces stations sont plus étroits et profonds que la référence de l'HER considérée.
...
```{r}
carhyce_hdf_der_ope %>% 
  drop_na(puissance_specifique_qb_w_m2) %>% 
ggplot( aes(x=version_protocole, y=puissance_specifique_qb_w_m2, 
                                       fill=version_protocole))+
  ...
  xlab("Version du protocole Carhyce")+
  ylab("Puissance spécifique moyenne à plein bord (en W/m2)")+
  scale_fill_brewer(palette="Set3")+
...
La valeur maximum (puissance spécifique de la dernière opération pour le Gland à Saint-Michel) parait aberrante aux vues du comportement du cours d'eau (observations des agents de terrain, spécifiquement lors des pêches et des Carhyce) et des faibles pentes de cours d'eau dans la région. 

```{r}
carhyce_hdf_der_ope %>% 
  drop_na(puissance_specifique_qb_w_m2) %>% 
ggplot( aes(x=Nom_HER_dom, y=puissance_specifique_qb_w_m2, 
                                       fill=Nom_HER_dom))+
  geom_boxplot(outlier.shape = NA)+
  geom_jitter(aes(color=classe_puissance))+
  stat_summary(data=carhyce_hdf_der_ope, fun.y=mean, geom="point", shape=20, size=6, 
...
Il sera à garder à l'esprit que le calcul de cette métrique comporte des incertitudes notables, à la fois vis à vis du calcul du débit de plein bord et du débit de crue biennale (incertitudes liées au calcul du Q2 à l'endroit de la station jaugée + incertitudes liées aux approximations faites lors de l'application de la formule de Myer). 
...
Qb_Q2_der_ope %>% 
  ...
  #geom_jitter()+
  stat_summary(fun.y=mean, geom="point", shape=20, size=6, color="red", fill="red") +
    geom_text(aes(label=..count..), y=0, stat='count', colour="blue", size=4, vjust=-29.5)+
...
Les mêmes observations peuvent être faites pour les distributions selon les HER 1. La majorité des valeurs sont > 1. Ceci indique que le débit de plein bord est supérieur au débit de crue biennale (crue morphogène), donc que la probabilité que le cours d'eau subisse une crue morphogène est moindre car le cours d'eau est profond. 

Ceci indique a priori une faible capacité des cours d'eau à évoluer.
...
On partira du principe, en accord avec les travaux hydromorphologiques de la Direction Générale de l'OFB et du CNRS (laboratoire de géographie physique), que plus la ripisylve est épaisse, haute et continue, plus son état peut être considéré comme bon. Ceci est relatif à une définition de l'hydromorphologie comme ensemble de processus physiques et non pas comme support de la biodiversité aquatique.
...
Il est décrit dans le rapport IED Carhyce de 2017 (Tamisier et. al) comme un potentiel indicateur pertinent du degré d'altération des cours d'eau.
...
```{r}
 carhyce_hdf_der_ope %>% 
  ggplot(aes(x=Nom_HER_dom, y=coefficient_de_sinuosite, fill=Nom_HER_dom))+
  geom_boxplot(outlier.shape = NA)+
  geom_jitter(aes(color=classe_sinuo))+
  stat_summary(fun.y=mean, geom="point", shape=20, size=6, color="red4", fill="red4") +
   geom_text(aes(label=..count..), y=0, stat='count', colour="blue", size=4, vjust = -68 )+
...
Après avoir observé la distribution des métriques étudiées pour les dernières opérations de chaque station en fonction des HER et des version de protocole (éventuellement selon des classes de métriques aidant l'interprétation), il est necessaire pour statuer sur un "état hydromorphologique global" dans les Hdf de définir des classes de qualité hydromorphologique (bon, mauvais etc.).
...
C'est à partir des ces intervalle que la classe de qualité est construite, en considerant que :
...
Ces résultats paraissent étonnants en comparaison avec le dire d'expert (connaissance des cours d'eau par les agents de l'OFB), qui tendrait plutôt à statuer sur un mauvais état, ou a minima un état moyen de la métrique largeur/profondeur de plein bord (cours d'eau incisés). Il a été constaté dans d'autres régions que le bon état de la géométrie hydraulique de certains cours d'eau incisés était surestimé par les modèles de référence, en particulier pour les cours d'eau de plaine et pour l'HER Tables calcaires. Les modèles de références pourraient donc être à l'origine de résultats peu représentatifs de la réalité de la géométrie hydraulique des cours d'eau des Tables calcaires, ce qui expliquerait le bon état étonnant de cours d'eau des Hauts-de-France pour la métrique largeur/profondeur. 
...
- une puissance > 35 W/m^2 révèle un état qui sera considéré comme "Bon" (classes de puissance 3,4 et 5)
- une puissance comprise en 20 et 35 W/m2 révèle un état qui sera considéré comme "Moyen" (classe de puissance 2)
...
Il semblerait donc, comme attendu, qu'au global les cours d'eau des Hauts-de-France puissent être caractérisés de peu voire très peu puissants, et par conséquent peu réactifs du point de vue de leur morphologie. Il existe donc potentiellement un risque vis à vis des altérations d'origines anthropiques sur ces cours d'eau, qui auraient a priori du mal à s'ajuster aux contraintes extérieures. 

## 3) Rapport Qb_Q2

Comme expliqué en partie II)3), si le rapport Qb_Q2 est inférieur à 1, cela signifie que le débit de plein bord est inférieur à la valeur de la crue morphogène, s'il est suppérieur à 1 c'est que le débit de plein bord est supérieur au débit de crue morphogène. 
...
Le choix de l'intervalle 0,8 ; 1,2 pour la bonne qualité (environ 1) est subjectif : il n'est pas possible de s'appuyer sur la littérature ou le dire d'expert pour définir cet intervalle. Mais la définition d'un bon état correpsondant à un rapport strictement égal à 1 parait trop strict au regard des incertitudes dans les calculs de la métrique. De plus, il est intéressant dans certains cas (lors de restauration de cours d'eau par exemple) que le débit de plein bord soit légèrement inférieur au débit de crue biennale (rapport Qb_Q2 légèrement inférieur à 1). 
 
Comme rappelé en partie II, du fait des estimations des débits de plein bord et des débits de crue biennale, la valeur Qb_Q2 comporte des incertitudes. 
...
Il existe plus de stations ayant un Qb_Q2 "Mauvais >1", qui est la situation la moins favorable. En effet, elle traduit une profondeur du cours d'eau telle que la probabilité de crue morphogène est diminuée, rendant les cours d'eau peu ajustables aux contraintes extérieures. De plus, elle traduit par conséquent des cours d'eau surcalibrés (surcreusement). Ceci va à l'encontre des résultats tirés de la métrique largeur/profondeur, et permet d'appuyer l'hypothèse selon laquelle les modèles de références pour la géométrie hydraulique surestiment le bon état des cours d'eau. 

## 4) Score ripisylve 

Comme expliqué en partie II)4), plus le score ripisylve est élevé, plus il est considéré que l'état de la ripisylve est bon. 
Sur l'IED Carhyce, et la correspondance entre Score ripisylve et ancienne classification de type de ripisylve. En se basant sur les valeurs globales de cette correspondance (médiane des distributions d'IMG en fonction des types de ripisyvles), les classes de qualité suivantes sont construites : 
...
Pour cette métrique, les résultats sont beaucoup plus contrastés, partagés majoritairement entre le bon et le moyen, avec également un nombre non négligeable de stations ne disposant pas de score ripisylve. 
...
Comme il était possible de le pressentir grâce aux distributions présentées en partie II)5), la majorité des stations carhyce présentent sur leur dernière opération un fort pourcentage de sédiments fins, > à 30%, qui les classent dans la catégorie "**Mauvais**" de la qualité relative au %fines. 

Il semblerait donc, selon ces classes de qualité, qu'une majorité de stations Crahyce dans les Hauts-de-France soient situées sur des cours d'eau au lit de faible qualité pour la biodiversité aquatique benthique et puissent être soumises à des risques de colmatages. Il convient une fois de plus de rappeler que ces seuils de qualité ne sont pas absolus et que de nombreux facteurs peuvent influencer la qualité de l'habitat benthique ou le colmatage du lit. D'ailleurs, la distribution des %fines des stations de références par HER (Partie II 5)) semble proche de celle des stations des Hauts-de-France. Des stations considérées comme peu influencées par les activités humaines peuvent donc aussi présenter des %fines élevés. Ceci peut amener à discuter plus avant des classes de qualité décrites ci-dessus. 

## 6) Coefficient de sinuosité

On partira du principe que plus un cours d'eau sinue, meilleure est son état regardant cette métrique. Les classes de qualité sont construites à partir des classes de sinuosité décrites dans le rapport IED Carhyce de 2017 (cf partie II)6)). Il est indiqué qu'un coefficient de sinuosité peut décrire une sinuosité forte au-delà de 1,25. Par conséquent, les classes suivantes sont créées : 
...
La classe de qualité dominante sur les dernières opérations des stations Carhyce est la classe Moyenne. De plus, la médiane de la distribution sur la région est de : 

```{r}
synthese_distri_met$Coef_sinuosite[1]
```

qui appartient à la classe Moyenne. 

L'état global de la sinuoisté en hdf est donc "**Moyen**". 

On aurait pu s'attendre en Hauts-de-France a avoir une qualité majoritairement mauvaise, étant donné que les cours d'eau de la région sont connus pour être fortement anthropisés. La qualité de la sinuosité montre des résultats un peu plus positifs qu'attendus, sans toutefois mettre en avant une bonne qualité de sinuosité. 

## 7)  Conclusion sur l'état global de l'hydromorphologie en Hdf 

A partir des métriques étudiées et caractérisées par rapport à des classes de qualité, il est possible à titre indicatif d'établir un état général de l'hydromorphologie dans la région. Evidemment, cet etat global, ne se basant que sur les 6 métriques choisies et sur des classes de qualité qu'il est possible de discuter, ne peut pas être représentatif de façon précise de l'état de l'hydromorphologie en Hauts-de-France. Il ne prétend pas faire office de diagnostic sur l'état hydromorphologique des cours d'eau de la région. 

Pour rappel, la qualité globale pour les métriques étudiées était : 

- Rapport largeur/profondeur de plein bord : Bon 
- Puissance spécifique de plein bord : Mauvais 
- Rapport Qb_Q2 : Mauvais 
- Score ripisylve : Moyen 
- %fines : Mauvais 
- Coefficient de sinuosité : Moyen 


```{r}
save(carhyce_hdf_der_ope2, file="processed_data/2_carhyce_hdf_der_ope_qualite_tot.RData")
```

D'après l'analyse faites des métriques choisies pour ce premier volet d'étude, on pourrait décrire globalement les cours d'eau prospectés par le protocole Carhyce comme **étant peu auto-ajustables et peu dynamiques, à la végétation rivulaire de qualité très variable, pouvant présenter un risque de colmatage ou de dégradation de la qualité de l'habitat benthique, mais dont la géométrie hydraulique (rapport largeur/profondeur de plein bord) semble peu altérée comparée aux références (mais résultats contradictoires avec le Qb_Q2)**.  