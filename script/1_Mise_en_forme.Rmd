...
Les exports IED Carhyce se font par HER 1, cours d'eau ou station. Les données importées initiallement seront donc celles de toutes les stations des HER 1 présentes en Hauts-de-France, à savoir :
...
IED_joint<-IED_joint %>% 
  ...
  rename(Date=date_realisation, # renommer certaines colonnes (noms plus courts)
...
carhyce_hdf_ofb<-carhyce_hdf_ofb %>% 
  mutate(Code_sta=as.character(Code_sta)) %>% 
   mutate(Code_sta = recode(Code_sta, "1090000" = "1000477", 
                            "1006000" = "1001131", 
                            .default = Code_sta),
         Libelle_sta = recode(Libelle_sta, "LA SLACK Ã€ AMBLETEUSE (62)" = "LA SLACK Ã€ RINXENT (62)", 
                              "L'HELPE MINEURE Ã€ MAROILLES (59)" = "HELPE MINEURE Ã€ GRAND FAYT (59)", 
                              .default = Libelle_sta)) 
```
...
Q2_fr<-Q2_fr %>%   
  ...
  rename(Code_sta= code_de_l_entite,
         ...
         Q2_m3 = debit_m_s) %>% 
  mutate(latitude = as.numeric(latitude), #changer de format de colonnes 
         longitude= as.numeric(longitude), 
         Code_sta= as.factor(Code_sta),
...
La fonction de calcul du débit de crue de période de retour T à l'endroit d'une stations pour laquelle on dispose d'une chronique de débits sur l'Hydro Portail necessitera d'avoir les informations suivantes :
