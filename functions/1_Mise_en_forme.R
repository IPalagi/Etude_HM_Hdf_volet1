...
function_Qb_Q2 <- function(code_region, Q2_fr, hydro_carhyce){
  ...
  #filtre Q2 sur la région : 
  
  sites_hdf<-info_sites_hydro$code_site #vecteur contenant les codes de sites hydro de la région selectionnée 
  
  Q2_fr<-Q2_fr %>% 
    filter(Code_sta %in% sites_hdf) # filtre des Q23 sur ces codes de sites (Attention aux éventuels codes de sites rassemblant plusieurs codes stations)
  ...
  hydro_carhyce_vf<-merge(carhyce_hdf_ofb_bv, hydro_carhyce, by="code_sta_HM")
  ...
  hydro_carhyce_vf<-merge(hydro_carhyce_vf, 
                          Q2_fr, 
                          by="code_site_hydro") 
